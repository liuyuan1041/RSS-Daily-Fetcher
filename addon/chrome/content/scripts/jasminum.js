...
(function () {
  ...
  async function createZoteroItem(feedItem, runCollections = null) {
    ...
    const targetCollectionID = parentCollection.id || parentCollection.collectionID;
    const existing = await findExistingItem(feedItem, targetLibraryID, targetCollectionID);
    if (existing) {
      let updated = false;

      const linked = await ensureItemInCollection(existing, parentCollection);
      if (linked) {
        updated = true;
      }
  ...
  async function ensureItemInCollection(item, collection) {
    if (!item || !collection) {
      return;
    }
    ...
    if (!Array.isArray(current) || !current.includes(collectionID)) {
  ...
  async function findExistingItem(feedItem, libraryID = null, collectionID = null) {
    ...
    if (typeof Zotero.Items.getByTitle === "function") {
      ...
      if (byTitle && byTitle.length > 0) {
        const sameLibrary = byTitle.find((it) => {
          ...
          return isItemInCollection(it, collectionID);
        });
        if (sameLibrary) {
          return sameLibrary;
        }
    ...
    const foundByTitleSearch = await findExistingBySearch(
      ...
      "title",
      feedItem.title,
      collectionID ? (it) => isItemInCollection(it, collectionID) : null
    );
  ...
  function isManagedItem(item) {
    ...
    return getExtraFieldValue(item, EXTRA_FIELD_MANAGED) === "1" || !!getExtraFieldValue(item, EXTRA_FIELD_SOURCE_KEY);
  }
  ...
  function ensureManagedMetadata(item, feedItem) {
    ...
    if (currentManaged !== "1") {
      changed = setExtraFieldValue(item, EXTRA_FIELD_MANAGED, "1") || changed;
    }
  ...
  function isItemInCollection(item, collectionID) {
    if (!item || !collectionID || typeof item.getCollections !== "function") {
      return false;
    }
  ...
  async function cleanupStaleItems(seenSourceKeys, summary = null) {
    if (!getPref("cleanupEnabled", true)) {
      if (summary) {
        summary.cleanupSkipped = "disabled";
      }
      return;
    }
    ...
    const rawChildren = typeof parentCollection.getChildItems === "function" ? parentCollection.getChildItems() : [];
    const childItems = (rawChildren || [])
      .map((it) => (typeof it === "number" ? Zotero.Items.get(it) : it))
      .filter((it) => !!it);
