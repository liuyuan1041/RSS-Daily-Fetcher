...
- 所有条目统一归类到目标父集合（默认：每日RSS论文）。
- 每次运行把新增条目放入父集合下的时间批次子集合（格式：`YYYYMMDD-HHMM`）。
- 自动将标题翻译并写入 `titleTranslation` 字段（即标题翻译列）。
- 支持定期自动清理目标父集合论文条目，默认 3 天。
...
2. 进入 工具 -> 附加组件。
3. 右上角齿轮 -> 从文件安装附加组件。
4. 选择当前目录下的 `rss-daily-fetcher-1.0.23.xpi`。
5. 安装后重启 Zotero。
...
- 新增批次结构：父集合下直接按运行时间创建批次集合，例如 `20260409-0930`、`20260409-1330`。
- 延迟清理逻辑：
  - 条目从 RSS 中消失时先标记缺失时间；
