...
- 自动将标题翻译并写入 `titleTranslation` 字段，同时兼容写入 `extra`。
- 支持定期自动清理论文条目，默认 3 天。
- 不包含“已读删除”功能：建议把感兴趣论文移动到其他集合管理。
- 不包含 PDF 抓取功能：建议配合 Zotero Connector 安装在浏览器中使用。
- 大模型API功能尚未实现。
- 不支持分类，关键词过滤。
- 要在ZOTERO中打开标题翻译列，否则无法显示翻译结果（论文条目顶部右击选中标题翻译）。
- 当前仅支持翻译标题，RSS接收的论文部分无摘要，因此未进行翻译。
...
2. 进入 工具 -> 附加组件。
3. 右上角齿轮 -> 从文件安装附加组件。
4. 选择当前目录下的 `rss-daily-fetcher-1.0.22.xpi`。
5. 安装后重启 Zotero。
